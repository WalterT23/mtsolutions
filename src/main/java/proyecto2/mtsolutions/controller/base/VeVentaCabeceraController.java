...
@Slf4j
@Controller
@RequestMapping(value = "/ve_venta_cabecera")
public class VeVentaCabeceraController {
    @Autowired
    private VeVentaCabeceraService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaVeVentaCabecera(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoVeVentaCabecera(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearVeVentaCabecera(HttpServletRequest request,
                                          @RequestBody VeVentaCabeceraDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearVeVentaCabecera(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}