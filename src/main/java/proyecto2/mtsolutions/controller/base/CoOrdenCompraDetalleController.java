...
@Slf4j
@Controller
@RequestMapping(value = "/co_orden_compra_detalle")
public class CoOrdenCompraDetalleController {
    @Autowired
    private CoOrdenCompraDetalleService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaCoOrdenCompraDetalle(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoCoOrdenCompraDetalle(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearCoOrdenCompraDetalle(HttpServletRequest request,
                                          @RequestBody CoOrdenCompraDetalleDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearCoOrdenCompraDetalle(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}