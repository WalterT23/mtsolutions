...
@Slf4j
@Controller
@RequestMapping(value = "/ve_timbrado")
public class VeTimbradoController {
    @Autowired
    private VeTimbradoService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaVeTimbrado(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoVeTimbrado(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearVeTimbrado(HttpServletRequest request,
                                          @RequestBody VeTimbradoDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearVeTimbrado(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}