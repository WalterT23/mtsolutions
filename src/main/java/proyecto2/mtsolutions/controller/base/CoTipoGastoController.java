...
@Slf4j
@Controller
@RequestMapping(value = "/co_tipo_gasto")
public class CoTipoGastoController {
    @Autowired
    private CoTipoGastoService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaCoTipoGasto(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoCoTipoGasto(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearCoTipoGasto(HttpServletRequest request,
                                          @RequestBody CoTipoGastoDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearCoTipoGasto(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}