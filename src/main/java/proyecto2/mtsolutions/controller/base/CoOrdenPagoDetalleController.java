...
@Slf4j
@Controller
@RequestMapping(value = "/co_orden_pago_detalle")
public class CoOrdenPagoDetalleController {
    @Autowired
    private CoOrdenPagoDetalleService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaCoOrdenPagoDetalle(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoCoOrdenPagoDetalle(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearCoOrdenPagoDetalle(HttpServletRequest request,
                                          @RequestBody CoOrdenPagoDetalleDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearCoOrdenPagoDetalle(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}