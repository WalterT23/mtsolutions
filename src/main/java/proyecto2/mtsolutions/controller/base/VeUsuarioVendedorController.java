...
@Slf4j
@Controller
@RequestMapping(value = "/ve_usuario_vendedor")
public class VeUsuarioVendedorController {
    @Autowired
    private VeUsuarioVendedorService service;

    @GetMapping(value = "/lista", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> listaVeUsuarioVendedor(HttpServletRequest request,
        ...
        resp.setData(this.service.listadoVeUsuarioVendedor(user, cantidad, origen));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }    @PostMapping(value="/crear", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crearVeUsuarioVendedor(HttpServletRequest request,
                                          @RequestBody VeUsuarioVendedorDTO dto) throws Exception {
        String user = request.getHeader(JWTServiceImpl.USER_STRING);
        log.info(request.getContextPath());
        MtSolutionsResponse resp = new MtSolutionsResponse();
        resp.setSuccess(true);
        resp.setData(this.service.crearVeUsuarioVendedor(dto, user));
        resp.setMessage("OK");
        return new ResponseEntity<>(resp, HttpStatus.OK);
    }
}