...
import proyecto2.mtsolutions.dao.base.CoOrdenCompraDAO;
...
@Slf4j
@Service
public class CoOrdenCompraServiceImpl implements CoOrdenCompraService {
    @Autowired
    private CommonUtils ctrl;

    @Autowired
    private CoOrdenCompraDAO coOrdenCompraDAO;

    @Override
    public ListasDTO listadoCoOrdenCompra(String usuario, int cantidad, int origen) throws Exception {
        try {
            ListasDTO lista = new ListasDTO();
            lista.setLista(coOrdenCompraDAO.getCoOrdenCompraList(cantidad, origen));
            lista.setTotalRegistros(coOrdenCompraDAO.getCoOrdenCompraListCount());
    ...
    @Override
    public String crearCoOrdenCompra(CoOrdenCompraDTO dto, String usuario) throws Exception {
        try {
            String re = "Registro creado correctamente";
            int xc = coOrdenCompraDAO.insertCoOrdenCompra(dto);
            if (xc==0) {
