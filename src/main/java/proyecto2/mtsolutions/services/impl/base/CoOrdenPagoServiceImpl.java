...
import proyecto2.mtsolutions.dao.base.CoOrdenPagoDAO;
...
@Slf4j
@Service
public class CoOrdenPagoServiceImpl implements CoOrdenPagoService {
    @Autowired
    private CommonUtils ctrl;

    @Autowired
    private CoOrdenPagoDAO coOrdenPagoDAO;

    @Override
    public ListasDTO listadoCoOrdenPago(String usuario, int cantidad, int origen) throws Exception {
        try {
            ListasDTO lista = new ListasDTO();
            lista.setLista(coOrdenPagoDAO.getCoOrdenPagoList(cantidad, origen));
            lista.setTotalRegistros(coOrdenPagoDAO.getCoOrdenPagoListCount());
    ...
    @Override
    public String crearCoOrdenPago(CoOrdenPagoDTO dto, String usuario) throws Exception {
        try {
            String re = "Registro creado correctamente";
            int xc = coOrdenPagoDAO.insertCoOrdenPago(dto);
            if (xc==0) {
