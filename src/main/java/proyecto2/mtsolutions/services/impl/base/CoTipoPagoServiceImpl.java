...
import proyecto2.mtsolutions.dao.base.CoTipoPagoDAO;
...
@Slf4j
@Service
public class CoTipoPagoServiceImpl implements CoTipoPagoService {
    @Autowired
    private CommonUtils ctrl;

    @Autowired
    private CoTipoPagoDAO coTipoPagoDAO;

    @Override
    public ListasDTO listadoCoTipoPago(String usuario, int cantidad, int origen) throws Exception {
        try {
            ListasDTO lista = new ListasDTO();
            lista.setLista(coTipoPagoDAO.getCoTipoPagoList(cantidad, origen));
            lista.setTotalRegistros(coTipoPagoDAO.getCoTipoPagoListCount());
    ...
    @Override
    public String crearCoTipoPago(CoTipoPagoDTO dto, String usuario) throws Exception {
        try {
            String re = "Registro creado correctamente";
            int xc = coTipoPagoDAO.insertCoTipoPago(dto);
            if (xc==0) {
