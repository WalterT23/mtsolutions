...
import proyecto2.mtsolutions.dao.base.CoBancoDAO;
...
import proyecto2.mtsolutions.services.base.CoBancoService;
...
@Slf4j
@Service
public class CoBancoServiceImpl implements CoBancoService {
    @Autowired
    private CommonUtils ctrl;

    @Autowired
    private CoBancoDAO coBancoDAO;

    @Override
    public ListasDTO listadoCoBanco(String usuario, int cantidad, int origen) throws Exception {
        try {
            ListasDTO lista = new ListasDTO();
            lista.setLista(coBancoDAO.getCoBancoList(cantidad, origen));
            lista.setTotalRegistros(coBancoDAO.getCoBancoListCount());
    ...
    @Override
    public String crearCoBanco(CoBancoDTO dto, String usuario) throws Exception {
        try {
            String re = "Registro creado correctamente";
            int xc = coBancoDAO.insertCoBanco(dto);
            if (xc==0) {
